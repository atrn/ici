...
ici_op_unary(void)
{
    ...
    default:
        switch (ici_opof(ici_xs.a_top[-1])->op_code)
        {
        case t_subtype(T_EXCLAM): return attempted("!", ici_typeof(ici_os.a_top[-1])->t_name);
        case t_subtype(T_TILDE): return attempted("~", ici_typeof(ici_os.a_top[-1])->t_name);
        case t_subtype(T_MINUS): return attempted("-", ici_typeof(ici_os.a_top[-1])->t_name);
        }
