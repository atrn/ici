...
static const char *opname(op *op)
{
    ...
    case OP_ASSIGN_TO_NAME:
        return "OP_ASSIGN_TO_NAME";
    case OP_ASSIGNLOCAL:
        return "OP_ASSIGNLOCAL";
    case OP_EXEC:
        return "OP_EXEC";
    case OP_LOOP:
        return "OP_LOOP";
    ...
    case OP_BINOP_FOR_TEMP:
        return "OP_BINOP_FOR_TEMP";
    case OP_AGGR_KEY_CALL:
        return "OP_AGGR_KEY_CALL";
    case OP_COLON:
        return "OP_COLON";
    case OP_COLONCARET:
        return "OP_COLONCARET";
    case OP_METHOD_CALL:
        return "OP_METHOD_CALL";
    case OP_SUPER_CALL:
        return "OP_SUPER_CALL";
    case OP_ASSIGNLOCALVAR:
        return "OP_ASSIGNLOCALVAR";
    case OP_CRITSECT:
        return "OP_CRITSECT";
    case OP_WAITFOR:
        return "OP_WAITFOR";
    ...
    case OP_SWITCHER:
        return "OP_SWITCHER";
    default:
        return "op by function";
    }
}

static void disassemble(int indent, array *a)
{
    object **e;
    char     n1[objnamez];
    int      i;
...
static int data_def(parse *p, objwsup *ows)
{
    ...
    for (;;)
    {
        ...
        case T_EQ:
            ...
            {
            case 0:
                not_followed_by("ident =", an_expression);
            case -1:
                goto fail;
            }
            hasinit = 1;
...
gather_string_or_re:
    ...
    case T_ONSQUARE:
        ...
        else if ((name = p->p_got.t_obj) == SS(class) || name == SS(map) || name == SS(module))
        {
            map *super;
            ...
            if (next(p, nullptr) == T_COLON || curtok == T_EQ)
            {
                ...
                if (!hassuper(o))
                {
                    set_error("attempt to do [%s %c %s", stringof(name)->s_chars, is_eq ? '=' : ':', objname(n, o));
                    decref(o);
                    goto fail;
                }
                if (is_eq)
            ...
            for (;;)
            {
                ...
                case T_ONROUND:
                    ...
                    {
                    case 0:
                        not_followed_by("[map ... (", an_expression);
                    case -1:
                        decref(d);
                        goto fail;
                    }
...
gotkey:
                    ...
                    else if (curtok == T_EQ)
                    {
                        ...
                        {
                        case 0:
                            not_followed_by("[map ... ident =", an_expression);
                        case -1:
                            decref(d);
                            goto fail;
                        }
...
static int unary(parse *p, expr **ep, int exclude)
{
    ...
    case T_DOLLAR:
        ...
        {
        case 0:
            set_error("badly formed expression");
        case -1:
            return -1;
        }
        if ((e = ici_talloc(expr)) == nullptr)
...
static int expression(parse *p, expr **ep, int exclude)
{
    ...
    while (t_type(next(p, nullptr)) == T_BINOP && curtok != exclude)
    {
        ...
        for (ep = ebase; (e = *ep) != elimit && tp < t_prec(e->e_what); ep = &e->e_arg[1])
        {
...
decl:
        ...
        if (p->p_got.t_obj == SS(case))
        {
            ...
            {
            case 0:
                not_followed_by("case", an_expression);
            case -1:
                return -1;
            }
        ...
        if (p->p_got.t_obj == SS(waitfor))
        {
            ...
            {
            case 0:
                not_followed_by("waitfor (", an_expression);
            case -1:
                return -1;
            }
            ...
            {
            case 0:
                not_followed_by("waitfor (expr;", an_expression);
            case -1:
                return -1;
            }
            if (a2->push_check(2))
...
const char *ici_token_name(int t)
{
    ...
    case t_type(T_NAME):
        return "name";
    case t_type(T_REGEXP):
        return "regexp";
    case t_type(T_STRING):
        return "string";
    case t_type(T_SEMICOLON):
        return ";";
    case t_type(T_EOF):
        return "eof";
    case t_type(T_INT):
    ...
    case t_type(T_FLOAT):
        return "float";
    case t_type(T_BINOP):
        return binop_name(t_subtype(t));
    case t_type(T_ERROR):
        return "error";
    case t_type(T_NULL):
        return "nullptr";
    case t_type(T_ONROUND):
        return "(";
    case t_type(T_OFFROUND):
        return ")";
    case t_type(T_ONCURLY):
        return "{";
    case t_type(T_OFFCURLY):
        return "}";
    case t_type(T_ONSQUARE):
        return "[";
    case t_type(T_OFFSQUARE):
        return "]";
    case t_type(T_DOT):
        return ".";
    case t_type(T_PTR):
        return "->";
    case t_type(T_EXCLAM):
        return "!";
    case t_type(T_PLUSPLUS):
        return "++";
    case t_type(T_MINUSMINUS):
        return "--";
    case t_type(T_CONST):
        return "const";
    case t_type(T_PRIMARYCOLON):
        return ":";
    case t_type(T_DOLLAR):
        return "$";
    case t_type(T_COLONCARET):
        return ":^";
    case t_type(T_AT):
        return "@";
    case t_type(T_BINAT):
