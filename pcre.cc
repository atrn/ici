...
static const char *OP_names[] = {
    "End",        "\\A",     "\\B",         "\\b",     "\\D",  "\\d",  "\\S",   "\\s",     "\\W",        "\\w",
    "\\Z",        "\\z",     "Opt",         "^",       "$",    "Any",  "chars", "not",     "*",          "*?",
    "+",          "+?",      "?",           "??",      "{",    "{",    "{",     "*",       "*?",         "+",
    "+?",         "?",       "??",          "{",       "{",    "{",    "*",     "*?",      "+",          "+?",
    "?",          "??",      "{",           "{",       "{",    "*",    "*?",    "+",       "+?",         "?",
    "??",         "{",       "{",           "class",   "Ref",  "Alt",  "Ket",   "KetRmax", "KetRmin",    "Assert",
    "Assert not", "AssertB", "AssertB not", "Reverse", "Once", "Cond", "Cref",  "Brazero", "Braminzero", "Bra"};
#endif
...
static BOOL compile_regex(int, int, int *, uschar **, const uschar **, const char **, BOOL, int, int *, int *,
                          compile_data *);
                        ...
                        compile_data *cd)
{
    ...
    else
    {
        ...
        case '0':
            c -= '0';
            while (i++ < 2 && (cd->ctypes[ptr[1]] & ctype_digit) != 0 && ptr[1] != '8' && ptr[1] != '9')
                c = c * 8 + *(++ptr) - '0';
            break;
...
static BOOL compile_branch(int options, int *brackets, uschar **codeptr, const uschar **ptrptr, const char **errorptr,
                           int *optchanged, int *reqchar, int *countlits, compile_data *cd)
{
...
OUTPUT_SINGLE_REPEAT:
            ...
            else if ((int)*previous >= OP_BRA || (int)*previous == OP_ONCE || (int)*previous == OP_COND)
            {
                int     i;
                int     ketoffset = 0;
                int     len = code - previous;
                uschar *bralink = nullptr;
                ...
                if (repeat_min == 0)
                {
                    ...
                    else
                    {
                        ...
                        *previous++ = OP_BRA;
                        ...
                        offset = (bralink == nullptr) ? 0 : previous - bralink;
                        bralink = previous;
                        *previous++ = offset >> 8;
                        *previous++ = offset & 255;
                    }
                ...
                if (repeat_max >= 0)
                {
                    for (i = repeat_max - 1; i >= 0; i--)
                    {
                        *code++ = OP_BRAZERO + repeat_type;
                        ...
                        if (i != 0)
                        {
                            int offset;
                            *code++ = OP_BRA;
                            offset = (bralink == nullptr) ? 0 : code - bralink;
                            bralink = code;
                            *code++ = offset >> 8;
                            *code++ = offset & 255;
                        }
                    ...
                    while (bralink != nullptr)
                    {
                        int     oldlinkoffset;
                        int     offset = code - bralink + 1;
                        uschar *bra = code - offset;
                        oldlinkoffset = (bra[1] << 8) + bra[2];
                        bralink = (oldlinkoffset == 0) ? nullptr : bralink - oldlinkoffset;
                        *code++ = OP_KET;
                        *code++ = bra[1] = offset >> 8;
...
END_REPEAT:
        ...
        case '(':
            ...
            if (*(++ptr) == '?')
            {
                ...
                default: /* Option setting */
                    ...
                    if (*ptr == ')')
                    {
                        if ((options & PCRE_INGROUP) != 0 && (options & PCRE_IMS) != (newoptions & PCRE_IMS))
                        {
                            *code++ = OP_OPT;
                            *code++ = *optchanged = newoptions & PCRE_IMS;
                        }
                        options = newoptions; /* Change options at this level */
                        previous = nullptr;   /* This item can't be repeated */
            ...
            previous = (bravalue >= OP_ONCE) ? code : nullptr;
            *code = bravalue;
            tempcode = code;

            if (!compile_regex(options | PCRE_INGROUP, /* Set for all nested groups */
                               ((options & PCRE_IMS) != (newoptions & PCRE_IMS)) ? newoptions & PCRE_IMS
                                                                                 : -1, /* Pass ims options if changed */
            ...
            if (subreqchar > 0 && (bravalue >= OP_BRA || bravalue == OP_ONCE || bravalue == OP_ASSERT ||
        ...
        case '\\':
            ...
            if (c < 0)
            {
                ...
                {
                    previous = (-c > ESC_b && -c < ESC_Z) ? code : nullptr;
                    *code++ = -c;
                }
                continue;
            }
...
static BOOL compile_regex(int options, int optchanged, int *brackets, uschar **codeptr, const uschar **ptrptr,
                          const char **errorptr, BOOL lookbehind, int condref, int *reqchar, int *countlits,
                          compile_data *cd)
{
    const uschar *ptr = *ptrptr;
    uschar       *code = *codeptr;
    ...
    *countlits = INT_MAX;
    ...
    for (;;)
    {
        ...
        if (!compile_branch(options, brackets, &code, &ptr, errorptr, &optchanged, &branchreqchar, &branchcountlits,
                            cd))
        {
            *ptrptr = ptr;
            return FALSE;
...
static BOOL is_anchored(const uschar *code, int *options)
{
    do
    {
        const uschar *scode = first_significant_code(code + 3, options, PCRE_MULTILINE, FALSE);
        int           op = *scode;
        ...
        else if ((op == OP_TYPESTAR || op == OP_TYPEMINSTAR) && (*options & PCRE_DOTALL) != 0)
        {
            if (scode[1] != OP_ANY)
                return FALSE;
        }
        else if (op != OP_SOD && ((*options & PCRE_MULTILINE) != 0 || op != OP_CIRC))
            return FALSE;
        code += (code[1] << 8) + code[2];
    } while (*code == OP_ALT);
    return TRUE;
...
static BOOL is_startline(const uschar *code)
{
    do
    {
        const uschar *scode = first_significant_code(code + 3, nullptr, 0, FALSE);
        int           op = *scode;
        if (op >= OP_BRA || op == OP_ASSERT || op == OP_ONCE || op == OP_COND)
        {
...
static int find_firstchar(const uschar *code, int *options)
{
    ...
    {
        int           d;
        const uschar *scode = first_significant_code(code + 3, options, PCRE_CASELESS, TRUE);
        int           op = *scode;
...
Arguments:
  ...
  tables       pointer to character tables or nullptr

Returns:       pointer to compiled data block, or nullptr on error,
               with errorptr and erroroffset set
*/

pcre *pcre_compile(const char *pattern, int options, const char **errorptr, int *erroroffset,
                   const unsigned char *tables)
{
    ...
    /* We can't pass back an error message if errorptr is nullptr; I guess the best we
    can do is just return nullptr. */
    ...
    while ((c = *(++ptr)) != 0)
    {
        ...
        case '[':
            ...
            do
            {
                if (*ptr == '\\')
                {
                    int ch = check_escape(&ptr, errorptr, bracount, options, TRUE, &compile_block);
                    if (*errorptr != nullptr)
                        goto PCRE_ERROR_RETURN;
                    if (-ch == ESC_b)
                        class_charcount++;
...
NORMAL_CHAR:
        default:
            ...
            while (runlength < 255 && (compile_block.ctypes[c = *(++ptr)] & ctype_meta) == 0);
    ...
    error, *errorptr will be set non-nullptr, so we don't need to look at the result
...
static BOOL match_ref(int offset, const uschar *eptr, int length, match_data *md, unsigned long int ims)
{
...
static BOOL match(const uschar *eptr, const uschar *ecode, int offset_top, match_data *md, unsigned long int ims,
                  BOOL condassert, const uschar *eptrb)
{
    ...
    for (;;)
    {
        ...
        case OP_COND:
            ...
            {
                if (match(eptr, ecode + 3, offset_top, md, ims, TRUE, nullptr))
                {
        ...
        case OP_ASSERTBACK:
            do
            {
                if (match(eptr, ecode + 3, offset_top, md, ims, FALSE, nullptr))
                    break;
                ecode += (ecode[1] << 8) + ecode[2];
            } while (*ecode == OP_ALT);
            if (*ecode == OP_KET)
        ...
        case OP_ONCE: {
            ...
            {
                if (match(eptr, ecode + 3, offset_top, md, ims, FALSE, eptr) ||
                    match(eptr, prev, offset_top, md, ims, FALSE, eptr))
                    return TRUE;
            }
            else /* OP_KETRMAX */
            {
                if (match(eptr, prev, offset_top, md, ims, FALSE, eptr) ||
                    match(eptr, ecode + 3, offset_top, md, ims, FALSE, eptr))
                    return TRUE;
            }
        }
            return FALSE;
        ...
        case OP_BRAZERO: {
            ...
            if (match(eptr, next, offset_top, md, ims, FALSE, eptr))
                return TRUE;
            do
        ...
        case OP_BRAMINZERO: {
            ...
            do
                next += (next[1] << 8) + next[2];
            while (*next == OP_ALT);
            if (match(eptr, next + 3, offset_top, md, ims, FALSE, eptr))
                return TRUE;
            ecode++;
        }
        break;
        ...
        case OP_DOLL:
            ...
            else
            {
                ...
                if (!md->endonly)
                {
                    if (eptr < md->end_subject - 1 || (eptr == md->end_subject - 1 && *eptr != '\n'))
                        return FALSE;
        ...
        case OP_NOT_WORD_BOUNDARY:
        case OP_WORD_BOUNDARY: {
            BOOL prev_is_word = (eptr != md->start_subject) && ((md->ctypes[eptr[-1]] & ctype_word) != 0);
            BOOL cur_is_word = (eptr < md->end_subject) && ((md->ctypes[*eptr] & ctype_word) != 0);
...
Arguments:
  ...
  external_extra  points to "hints" from pcre_study() or is nullptr
...
int pcre_exec(const pcre *external_re, const pcre_extra *external_extra, const char *subject, int length,
              int start_offset, int options, int *offsets, int offsetcount)
{
    ...
    const uschar          *start_bits = nullptr;
    ...
    if (re == nullptr || subject == nullptr || (offsets == nullptr && offsetcount > 0))
        return PCRE_ERROR_NULL;
    if (re->magic_number != MAGIC_NUMBER)
        return PCRE_ERROR_BADMAGIC;
    ...
    if (re->top_backref > 0 && re->top_backref >= ocount / 3)
    {
        ...
        if (match_block.offset_vector == nullptr)
    ...
    if (!anchored)
    {
        ...
        else if (!startline && extra != nullptr && (extra->options & PCRE_STUDY_MAPPED) != 0)
    ...
    if ((re->options & PCRE_REQCHSET) != 0)
    {
        ...
        req_char2 =
            ((re->options & (PCRE_CASELESS | PCRE_ICHANGED)) != 0) ? (re->tables + fcc_offset)[req_char] : req_char;
    }
    ...
    do
    {
        ...
        {
            if ((ims & PCRE_CASELESS) != 0)
                while (start_match < end_subject && match_block.lcc[*start_match] != first_char)
                    start_match++;
            else
    ...
    while (!anchored && match_block.errorcode == PCRE_ERROR_NOMATCH && start_match++ < end_subject);
