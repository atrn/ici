...
{
    if ((ici_objof(h)->o_flags & ICI_H_CLOSED) == 0)
...
ici_net_socket(void)
{
    ...
    return ici_ret_with_decref(ici_objof(skt));
}
...
ici_net_close(void)
{
    ...
    ici_objof(skt)->o_flags |= ICI_H_CLOSED;
    return ici_null_ret();
}
...
ici_net_accept(void)
{
    ...
    return ici_ret_with_decref(ici_objof(new_netsocket(fd)));
}
...
ici_net_connect(void)
{
    ...
    unblock(x);
    return rc == -1 ? ici_get_last_errno("net.connect", NULL) : ici_ret_no_decref(ici_objof(skt));
}
...
ici_net_bind(void)
{
    ...
    if (ICI_NARGS() == 2)
    {
        ...
        if (!ici_ishandleof(skt, ICIS(socket)))
    ...
    if (set != NULL)
    {
        for (i = 0; *n > 0 && i < set->s_nslots; ++i)
        {
            ...
            if (!ici_ishandleof(sl->sl_key, ICIS(socket)))
                continue;
            fd = (SOCKET)ici_handleof(sl->sl_key)->h_ptr;
            if (FD_ISSET(fd, fds))
            {
                --*n;
...
ici_net_select()
{
        ...
        else if (ici_isset(ICI_ARG(i)) || ici_isnull(ICI_ARG(i)))
        {
            ...
            if (ici_isset(ICI_ARG(i)))
            {
                ...
                for (n = j = 0; j < set->s_nslots; ++j)
                {
                    ...
                    k = (SOCKET)ici_handleof(sl->sl_key)->h_ptr;
                    FD_SET(k, fs);
                    if (k > dtabsize)
                        dtabsize = k;
                    ++n;
    ...
    if (select_add_result(result, ICIS(except), eset, efds, &n))
        goto fail;
    return ici_ret_with_decref(ici_objof(result));

fail:
    ici_decref(ici_objof(result));
    return 1;
}
...
static ici_cfunc_t cfuncs[] = 
{
    ...
    {ICI_CF_OBJ, "listen", ici_net_listen},
    {ICI_CF_OBJ, "accept", ici_net_accept},
    {ICI_CF_OBJ, "connect", ici_net_connect},
    {ICI_CF_OBJ, "bind", ici_net_bind},
    {ICI_CF_OBJ, "select", ici_net_select},
    {ICI_CF_OBJ, "sendto", ici_net_sendto},
    {ICI_CF_OBJ, "recvfrom", ici_net_recvfrom},
    {ICI_CF_OBJ, "send", ici_net_send},
    {ICI_CF_OBJ, "recv", ici_net_recv},
    {ICI_CF_OBJ, "getsockopt", ici_net_getsockopt},
    {ICI_CF_OBJ, "setsockopt", ici_net_setsockopt},
    {ICI_CF_OBJ, "hostname", ici_net_hostname},
    {ICI_CF_OBJ, "getpeername", ici_net_getpeername},
    {ICI_CF_OBJ, "getsockname", ici_net_getsockname},
    {ICI_CF_OBJ, "getportno", ici_net_getportno},
    {ICI_CF_OBJ, "gethostbyname", ici_net_gethostbyname},
    {ICI_CF_OBJ, "gethostbyaddr", ici_net_gethostbyaddr},
    {ICI_CF_OBJ, "sktno", ici_net_sktno},
    {ICI_CF_OBJ, "sktopen", ici_net_sktopen},
    {ICI_CF_OBJ, "shutdown", ici_net_shutdown},
#ifndef USE_WINSOCK
    {ICI_CF_OBJ, "socketpair", ici_net_socketpair},
#endif
    {ICI_CF_OBJ}
};
