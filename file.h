...
struct file : object
{
    ...
    inline int getch()
    {
        return f_type->getch(f_file);
    }
    inline int ungetch(int c)
    {
        return f_type->ungetch(c, f_file);
    }
    inline int flush()
    {
        return f_type->flush(f_file);
    }
    inline int close()
    {
        return f_type->close(f_file);
    }
    inline long seek(long o, int w)
    {
        return f_type->seek(f_file, o, w);
    }
    inline int eof()
    {
        return f_type->eof(f_file);
    }
    inline int read(void *p, long n)
    {
        return f_type->read(p, n, f_file);
    }
    inline int write(const void *p, long n)
    {
        return f_type->write(p, n, f_file);
    }
    inline int fileno()
    {
        return f_type->fileno(f_file);
    }
    inline int setvbuf(char *p, int t, size_t z)
    {
        return f_type->setvbuf(f_file, p, t, z);
    }
...
inline file *fileof(object *o)
{
    return o->as<file>();
}
inline bool isfile(object *o)
{
    return o->hastype(TC_FILE);
}
